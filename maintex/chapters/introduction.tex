...
con la filosofía, en particular filosofía de la ciencia.\footnote{
...
Para apreciar la discusión en torno a las motivaciones de
CA, voy a comenzar exponiendo brevemente en qué consiste el
modelo de Hempel $\&$ Oppenheim. Este va a ser el punto de
partida desde el cual quiero trazar una historia del papel
que, me parece, juega la causalidad en una teoría de la
explicación científica. Elijo este punto de partida porque
(i) es una teoría clásica en filosofía de la ciencia y
porque en este capítulo, (ii) pretendo justificar que
...
explicaciones no son simétricas. Más controvertido es
afirmar que son \emph{relaciones causales} lo que explica perfectamente
...
títulos mencionados en el párrafo anterior, terminando esta
sección con una caracterización de la teoría \emph{intervencionista} de
la causalidad.
...
en ofrecer un argumento deductivo. En principio, este análisis suena
sumamente plausible, sin embargo, hay un par de detalles que
merman su plausibilidad.
...
$ \$1 $. La conexión que hay en el ejemplo del cobre es algo
que le falta al ejemplo de las moendas.
...
disciplinas científicas  dispongan de leyes naturales para
ofr%ecer explicaciones.\footnote{
Para% referencia véase \parencite{dieguez2012}}.
Pero aún si podemos
caracterizar a una ciencia cualquiera que use leyes, todavía es
necesario definir el comportamiento de la
relación entre las leyes naturales y los
...
%\para%graph{Sylvain Bromberger (\citeyear{Bromberger1966})}
...
Sin embargo, si  enmarcamos el problema en los términos que
presenta el autor, entonces lo que llama \say{la doctrina
...
que debería explicar la persona que diseñó el edificio. El
problema no es el argumento, sino de quién afirme que dicha
...
%este,y otros contraejemplos en \citeyear{Bromberger1966}, pero
su propósito no era mostrar que el modelo
nomológico-deductivo implica que la explicación es
asimétrica,\footnote{
...
Pero para continuar la historia de la condición de
asimetría, en lo siguiente,  me voy a concentrar en el
modelos de explicación de Wesley Salmon, quien empleó el contraejemplo que presentó
Bromberger, y mostró que el modelo por ley de cobertura
implica simetría. Salmon, por supuesto, no sólo se dedicó a
señalar los problemas del modelo por ley de cobertura, sino
que presenta una teoría sustancial de
la explicación científica que lidia con la
...
señalando el problema de la simetría y afirma que la
asimetría temporal es crucial para satisfacer CA. Polemizando el contraejemplo de
...
Esto, por supuesto, resolvería nuestro problema, porque
primera vista, es obvio que los procesos físicos tienen
...
Entonces suena sensato pensar que cualquier explicación que podamos
ofrecer de un fenómeno, debería respetar esta dirección
temporal.

Sin embargo, basar la asimetría de la explicación en la
asimetría temporal de los procesos físicos comienza a
parecer un compromiso metafísico que Hempel y Oppenheim
buscaban evitar: comienzan a aparecer las relaciones
causales en este análisis. Pero
Hempel y Oppenheim---se afirma tradicionalmente---buscaban
excluir a dichas relaciones del análisis de la explicación científica, porque la
causalidad es una noción oscura y metafísicamente cargada.
Es por ello que hay que dar más detalles para justificar
esto. Para hacer
...
la asimetría temporal como base de la asimetría de la explicación.
...
icosaédrico, sabemos que el dado sólo va a mostrar una de sus caras en
...
capaces de incluir todas las variables pertinentes. Por último, las
...
leyes---la dilatación de los metales--- y generalizaciones
accidentales---mi pobreza. Además, evitar que la
...
que haya tal cosa como leyes naturales (como la biología o la economía).
...
la velocidad, ambas estadísticamente relevantes.
...
CA, porque Salmon ha basado la condición de asimetría en la dirección temporal.
...
relevantes por separado---como en $D'$.
...
demás condiciones permanecen constantes---no
...
explicaciones genuinas.\footnote{
...
tiene en su núcleo a las relaciones causales. La
teoría de Woodward se ha llamado \emph{manipulabilista} o
\emph{intervencionista} y esta teoría tiene una serie de ventajas que describiré más
...
para que esto cuente como relación causal, el cambio en el
valor de la variable debe ser sólo a través del cambio de
otra variable
particular. Esto implica que en un caso particular,
deberíamos ser capaces de aislar todas las variables
distintas a la presión atmosférica. Nos concentramos en una
variable particular porque es aquella está bajo escrutinio.
Buscamos el aporte causal que hace el cambio en la presión atmosférica, no otras
variables.

Esto, por supuesto, sólo es una parte de la historia de las
teorías manipulabilistas. Esta teoría tiene fuertes
implicaciones en torno a cómo se enmarca la búsqueda de
relaciones causales en investigación científica. El ejemplo
de la lluvia funciona para señalar algunas de estas
consecuencias.

La teoría implica, por ejemplo, que los sistemas que
ostentan relaciones causales son sistemas totalmente modulares. Esto
quiere decir, que podemos ser capaces de
aislar todas y cada una de las variables del sistema. Digamos que queremos explicar qué aporte causal que tiene la
presión atmosférica en $D$. $D$ está construido de tal
manera que lo que resulta relevante para explicar por qué
llueve es sólo la velocidad del viento. Por otro lado, en $D'$ nos preguntamos
no sólo por el aporte causal de la
velocidad del viento, sino también los cambios en la presión
atmosférica. Ambas descripciones claramente
implican una diferencia en cómo tratamos a las variables
involucradas en el sistema.

Este compromiso con la modularidad de los sistemas implica
que es posible distinguir claramente entre ambas descripciones y ajustar el modelo en consecuencia. Dado que el sistema es
modular, seremos capaces de aislar las variables relevantes, siempre
independientes de cualquier otra variable del sistema. Sin
embargo, no todos los sistemas son modulares, como argumenta Cartwright en %\parencite{Cartwright2002}. Lo
que quiero resaltar al mencionar estas implicaciones es que
la teoría no es la panacea y tiene compromisos
problemáticos. Por el momento dejaré la discusión de los problemas que tiene
la teoría manipulabilista, porque voy a presentar en la sección \ref{s:pii}
un problema más grave en torno a las implicaciones de la
teoría, que además es el problema que me interesa discutir en este proyecto de
investigación.

Para fines explicativos, continuando con la presentación de
la teoría manipulabilista, por ahora voy a formular
un ejemplo más manejable, intentando evitar
estas implicaciones. Olvidémonos por un momento de la
...
producen \emph{6 metros de distancia} de un proyectil. Como
la variable bajo escrutinio es el aporte causal de la
fuerza aplicada, supongamos que esta función es
\emph{independiente} de cualquier fuerza externa. Por
ejemplo, la resistencia del medio---en este caso el aire---o
el ángulo de lanzamiento, porque dependiendo del ángulo, la gravedad
comienza a jugar un papel. Si logramos satisfacer estas
...
función definida anteriormente describa esta relación
...
Quiero recordar al lector el problema que he estado
discutiendo hasta aquí: que las
relaciones causales son centrales para resolver CA. El
modelo de Salmon resuelve muchos de los problemas de $H \&
O$, sin embargo, si el modelo no incluye factores causales,
entonces falla en hacer distinciones cruciales entre
diferentes sistemas. Si esto
...
también esté convencido de esto.
...
débil, porque se basa en las virtudes que tiene la teoría
para dar cuenta de la explicación causal en investigación
científica. Si el siguiente argumento a favor de la teoría
manipulabilista tiene una forma, lo llamaría un argumento de
\emph{indispensabilidad metodológica,} y, por tanto, también
señalaré los compromisos metodológicos de la teoría
%\parencite{Quine1961} (en analogía con el
argumento de indispensabilidad). Lo que
pretendo en la siguiente sección es mostrar que es plausible asumir que la teoría
manipulabilista de la causalidad es la teoría que debemos
asumir si queremos una buena caracterización de la
causalidad. Para llegar a esta conclusión, el argumento que
daré en la siguiente sección descansa en
señalar las virtudes de la teoría al momento de analizar
hipótesis causales en investigación científica---tal como
suelen suceder en investigación científica. Me parece que
con esto, la teoría paga paga su cuota.
...
estarán poblados de ejemplos de la literatura
contemporánea que asumen una teoría manipulabilista de la
causalidad. Diré que estos ejemplos de la adopción de la
teoría y las virtudes que tiene, son suficientes para que la
teoría pague su \emph{cuota metodológica.}
...
manipulabilista: por un lado que el énfasis en la
manipulabilidad hace que la teoría sea
\emph{antropocéntrica}, por otro lado, se ha mencionado
que el concepto de causa del cuál echa mano la teoría es
inevitablemente \emph{circular.} Después de despachar ambas
razones, señalaré por qué la adopción de la teoría en un
...
\noindent Para este punto de mi anécdota, me parece que he
cimentado mi intuición de que una teoría de la explicación
debe incluir un factor causal para satisfacer CA. Afirmar que la mejor teoría de la explicación que satisface este criterio es la teoría \emph{manipulabilista,} es una afirmación que requiere razones. En esta sección voy a ofrecer las razones por las cuáles voy a
\emph{adoptar} a la teoría manipulabilista, como un marco
metodológico, en particular la
\emph{caracterización de la causalidad} que ésta ofrece. Sé
que la teoría \emph{manipulabilista} no es la panacea, por
esto que creo que adoptar esta teoría de la explicación, sin
cambios sustanciales, lleva a conclusiones indeseables. Sin embargo, me parece que estas conclusiones
indeseables sólo se obtienen si interpretamos a la teoría
%como lo hace Woodward en \citeyear{caushuman}. Para lograr
mi objetivo, primero caracterizaré  problema en los mismos términos que
ofrece el autor. Luego señalaré por qué este no es un
problema sólo para la teoría manipulabilista, sino un
problema más general para el debate en torno a la
\emph{explicación causal.}
Llamaré a este problema \emph{la deuda normativa.}
...
lo voy a hacer aquí. Estas cuestiones metafísicas de la
causalidad son sumamente pertinentes para una
caracterización de la explicación causal. Sin embargo, discutir
esto me aleja del objetivo de este proyecto. No obstante,
creo que los compromisos causales de la teoría, tal como los voy a
presentar, son lo suficientemente neutrales ante estos
problemas. A lo largo de este trabajo, voy a asumir que las relaciones son
variables-\emph{type}, por razones que quedarán más claras
en \ref{s:piii}, donde explico lo que
se ha llamado \emph{el problema fundamental de la inferencia
causal}

Dicho lo anterior, me parece claro que la literatura
contemporánea ha adoptado una postura manipulabilista de la
causalidad, no sólo en filosofía, sino también en
investigación científica. Potochnik, hablando en particular
de la causalidad, señala que debemos incluir factores
causales si es que queremos una visión más parsimoniosa de
la ciencia, Potochnik nos dice que \say{[$\ldots$] defiendo
que la búsqueda de patrones causales es la manera provechosa
de entender a la ciencia.}\footnote{
...
Este \say{espíritu causal manipulabilista} no sólo aparece
en la literatura filosófica, sino también en libros de texto
que discuten el papel que juega la causalidad en la
investigación científica. Por ejemplo, cabe notar que en
%\parencite{Pearl2016}, los autores  inician señalando que la
causalidad \emph{debería} ser empleada en nuestras
explicaciones estadísticas, los autores comentan que:
...
teoría ha \say{pagado} su cuota metodológica. Sin embargo, a
pesar de esto, la teoría manipulabilista padece de un
problema más grave al cuál llamaré la \say{deuda normativa.}
En la siguiente sección me dedicaré describir el problema.
La sección \ref{s:piii} la dedico a señalar por qué el
problema es importante y cómo este trabajo de investigación
se presenta como una respuesta a este problema.

Para comenzar, voy a discutir un trabajo reciente
%\citetitle{caushuman}, en el cuál Woodward ha defendido el
papel que juega su teoría en el debate filosófico en torno a
la explicación científica. Me parece que el autor despacha
muy rápidamente la cuestión de como interpretar la
normatividad de su teoría y, me parece también, que la
interpretación que ofrece Woodward no es suficiente para
resolver el problema al que se enfrenta. La razón por la
cuál no es suficiente es porque da paso a una mala
interpretación filosófica de la ciencia. Si lo que el autor
dice es verdad, tendremos conclusiones indeseables en
nuestra mejor teoría de la explicación científica. Llamaré a
esto el problema de la \emph{deuda normativa.}

Para presentar claramente el problema, comienzo
caracterizando las tesis que sostiene Woodward en
\emph{Causation with a Human Face} y luego problematizo la
...
en particular, me quiero concentrar en la distinción entre
lo \emph{descriptivo} y lo \emph{normativo}. Woodward
apuesta a que la distinción es espuria si entendemos que hay
una brecha entre ambos dominios. La respuesta del autor es
que no existe tal brecha y ofrece una serie de argumentos
mediante los cuales traza un puente entre lo normativo y lo
descriptivo señalando que ambos dominios deben estar
...
dado que estoy asumiendo que la función está bien definida y
es correcta. Pero es completamente seguro de que será
necesario definir una función distinta para velocidades
cercanas a la velocidad de la luz, donde el espacio-tiempo
se curva. Entonces decimos que esta función es
\emph{invariante} y \emph{proporcional} para valores desde
al menos desde el 1, hasta valores siempre debajo de la
velocidad de la luz. Por otra parte, la función que describe
el precio de la gasolina durante los últimos 20 años,
ajustado a la inflación, puede ser verdadera para un rango
de valores. Sin embargo, la función no es proporcional
porque siempre hay que ajustar el porcentaje de inflación de
cada año (que fluctúa por año).
...
conclusión, entonces la otra opción es, o bien buscar otras
relaciones \emph{normativamente adecuadas} que describa una
teoría diferente del razonamiento causal.
...
la caracterización de la causalidad que hace la teoría
manipulabilista, depende de una caracterización de los
\emph{condicionales contrafácticos.} Comienzo la siguiente
sección señalando qué papel desempeñan los contrafácticos en
la teoría manipulabilista y, más general, el papel que
juegan los contrafácticos en nuestra teoría de la
\emph{explicación científica.} Al final diré por qué esta
tesis se presenta como una solución al problema general de
la \emph{deuda normativa.}


\section{Parte III}\label{s:piii}


\noindent Hasta este momento, me he concentrado en repasar
rápidamente algunas de las teorías de la explicación
clásicas en filosofía de la ciencia. Esto lo hice con el
objetivo de señalar cómo la causalidad fue involucrándose en
el debate de la explicación científica. Después de esta
caracterización---caracterización mayormente
histórica---motivé que nuestra teoría de la explicación
debe tener un componente causal si es que queremos
satisfacer el criterio de asimetría.

La siguiente sección del presente capítulo, la dediqué a
señalar en que consiste la teoría de la explicación
manipulabilista y por qué---al menos como hipótesis
metodológica---voy a asumirla como la teoría correcta de
la explicación científica: porque paga su \emph{cuota
metodológica.} A partir de la caracterización de la teoría
manipulabilista y el proyecto reciente de Woodward, me
dediqué a presentar un problema al cuál se enfrenta esta
teoría, a saber, la \emph{deuda normativa.} Me parece que
este problema no sólo aqueja a la teoría manipulabilista,
sino que el problema es más general, esto es, que es un
problema para cualquier teoría que pretenda basar los
criterios \emph{normativos} en términos puramente
\emph{descriptivos.} En breve, argumenté que adoptar una
teoría del razonamiento y tratar de adaptarla para resolver
cuestiones normativas, tiene consecuencias
indeseables.\footnote{
  No creo que la tarea sea imposible, sino que debería
  hacerse con cuidado. Woodward ciertamente no
  tiene cuidado cuando discute que su teoría sirve de base
  normativa para trazar relaciones causales. Un claro
  ejemplo de cómo se llevaría con mas cuidado y éxito este
%  proyecto, lo hace Roussos en \citetitle{roussos}
%}
Si esto es verdad, entonces aún queda por saldar lo que llamé
la \emph{deuda normativa.}

Quiero poner mis cartas en la mesa y señalar algunas de las
suposiciones que voy a hacer a lo largo de este trabajo.
Como confesé al inicio, estoy asumiendo que la causalidad
resuelve CA. Además, mencioné que me parece que la teoría
manipulabilista es la caracterización correcta de la
causalidad. Sin embargo, también creo que la teoría falla si
la interpretamos en las líneas que presenta Woodward, esto
es, como una teoría \emph{normativa.} A pesar de esta deuda,
me parece que es un error echar por tierra todo el proyecto
que presenta Woodward. Esto sería tirar al bebé con todo y
bañera. Es por eso que dediqué una parte a señalar su
importancia en la investigación tanto filosófica, como
científica.

Ahora bien, no lo he hecho explícito, pero si la teoría
manipulabilista me va a servir como un marco, tengo que
comprometerme con al menos algunas de las implicaciones de
la teoría. Como además me parece que es una teoría adecuada
de la \emph{explicación causal,} tengo que comprometerme
explícitamente con la caracterización de la causalidad que
ofrece la teoría y dicha caracterización depende de lo que
en filosofía se han llamado \emph{condicionales
contrafácticos.}

Los \emph{condicionales contrafácticos} tienen un rol
central en la teoría manipulabilista, sin embargo, quiero
enfatizar que estos condicionales tienen un rol más general
en la investigación científica, particularmente en los
métodos de \emph{inferencia causal.} Si esto es verdad
entonces los contrafácticos tienen un papel independiente de
la teoría manipulabilista. Este papel inferencial es el
que que me parece que vale la pena analizar: su función
en la explicación causal.

En esta tercera parte del capítulo, procedo de la siguiente
manera: la primera parte la dedico a hacer explícitos
algunos de los supuestos que estoy adoptando. Después explico el papel que
ocupan los \emph{condicionales contrafácticos} en la teoría
de Woodward y el rol que tienen en investigación científica
en general. Lo siguiente que haré será caracterizar el
problema que se ha llamado \emph{problema fundamental de la
inferencia causal} y por qué es pertinente un marco
normativo para echar luz sobre este problema. Por último, señalo como esta tesis se presenta como
un marco que echa luz en torno a cómo resolver este
problema.

\paragraph{Algunos de los \emph{desiderata}} que quiero
satisfacer,  son muy cercanos a los que asume Woodward y si
la teoría manipulabilista va a ser mi marco, creo que es
relevante hacerlos explícitos. Siguiendo al autor, parece
que si una teoría va a servir como base \emph{normativa,}
dicha teoría de la explicación tiene que dar cuenta de cómo
\emph{de hecho se practica} en ciencias empíricas. La teoría
además tiene que dar cuenta de los \emph{límites cognitivos}
de los seres humanos y explicar cómo las explicaciones son
\emph{usadas} con ciertos propósitos en mente. Al menos eso
parece señalar el autor cuando dice que:

\begin{quote}
  My approach in this book involves a different way of
  thinking about causation [$\ldots$] It involves thinking
  about causal notions in \emph{funcitonal} terms, that is,
  focusing on what we want to \emph{do} with such notions,
  with the function or goals or purposes causal thinking
  serves for agents like us [$\ldots$] %\parencite[Énfasis en
 % el original,][p.~18]{caushuman}
\end{quote}


Además, siguiendo al autor, el proyecto normativo consiste
en ofrecer una teoría que lidie con cómo deberíamos
razonar, o en sus palabras \say{[$\ldots$]
explícitamente \say{normativo} en el sentido en el cuál
trata de describir como \emph{deberíamos} aprender, razonar,
y juzgar relaciones causales.} %\parencite[Énfasis en el
%original][p.~21]{caushuman}\footnote{
%En el original \say{[$\ldots$] this work from
%outside of philosophy is explicitly \say{normative} in the
%sense that it attempts to describe how we \emph{ought} to
%learn, reason, and judge about causal relationships.}
%}

Para motivar estos compromisos, Woodward presenta un dilema para los filósofos que quieran
perseguir un proyecto normativo: o
bien debemos negar los datos empíricos que ofrecen evidencia
de cómo razonamos, o bien conceder los datos empíricos y,
afirmar como cuestión normativa, que al hablar de causalidad
tal como existe en el mundo (o sobre lo que la causalidad
\say{es}), estamos apuntando hacia una caracterización de la
causalidad que se abstrae totalmente de influencias
humanas. Una caracterización de las relaciones causales,
tal como dios las describiría, o algo por el estilo. El
primer cuerno del dilema es uno que no voy a discutir. Me
parece obvio que no deberíamos desechar ningún tipo de
evidencia empírica, porque hacerlo sería \emph{ad hoc} a la
teoría. Es claro que la evidencia psicológica debe
informar cómo debemos razonar, en especial si queremos
%evitar sesgos \parencite{nordell2021end}. El segundo cuerno
del dilema, explica el autor, lleva a compromisos
metafísicos demasiado alejado de la práctica científica.

Woodward también enfatiza que incluir este tipo de factores
humanos, no implica ningún tipo de subjetivismo o
proyectivismo acerca de la causalidad. El autor incluso
insiste en que \say{$\ldots$ the \say{interventionist}
account of causation that I favor is also normative in the
sense that it makes recommendations about which causal
claims should be judged true or false $\ldots$}
%\parencite[p.~22]{caushuman}.

Esto significa que, por un lado, una teoría normativa tiene
que lidiar con los \emph{usos,} \emph{límites} y
\emph{prácticas} de la explicación científica, mientras que, por otro lado, tiene
que dar cuenta de por qué las explicaciones son usualmente
exitosas. Donde exitosas significa que estas
explicaciones rastrean relaciones causales tal como son en
el mundo. Si la teoría caracteriza a la causalidad en términos
de \emph{condicionales contrafácticos,} entonces, la
teoría tiene que decirnos cómo \emph{deberíamos} razonar
para saber si dichos condicionales son verdaderos o falsos.
Pero la teoría no puede hacer esto porque falla en su
empresa normativa.

A pesar de esto, creo que el proyecto
normativo que persigue Woodward, con los supuestos
anteriores, es pertinente para caracterizar correctamente a
la explicación científica. En especial,
me parece que lo que se necesita es una teoría de la
\emph{justificación} que resuelva las tensiones que hay
entre estos supuestos y la base \emph{normativa.} Lograr
resolver estas tensiones \emph{es el objetivo de este
proyecto.}

Antes de continuar con los alcances y límites de mi
proyecto, quiero recordar al lector que hasta este punto he estado hablando de \emph{condicionales
contrafácticos} sin decir más sobre qué son. Me parece que
para apreciar los alcances y límites de mi proyecto, primero
es necesario caracterizar a estos condicionales. Los
siguientes párrafos los dedico a esto, y a resaltar el papel
que juegan en la investigación científica en general.

La caracterización de la causalidad que hace la
la teoría manipulabilista depende de una interpretación
particular de los \emph{condicionales contrafácticos.} Como
he dicho al final de la sección anterior, si todo lo que
dice el autor es verdad y el problema de la \emph{deuda
normativa} está bien motivado, se sigue que el proyecto
normativo \emph{debería} decirnos cómo es que dichos
condicionales son \emph{verdaderos} o \emph{falsos.} Hasta
este punto de la historia, he tratado de convencer al lector
de que la teoría manipulabilista no es exitosa en su
\emph{empresa normativa,} por lo tanto, no ofrece criterios
suficientes para informarnos si estamos justificados en
decir que existe una relación causal entre dos variables
cualesquiera $A$ y $B$, donde esta relación entre $A$ y $B$
está definida apelando a estos condicionales.

\paragraph{Los condicionales contrafácticos} son aquellos
condicionales, cuyo antecedente evaluamos asumiendo que es
\say{contrario a los hechos.} Que algo que de \emph{hecho}
sucede, no sucede en absoluto (sé que la oración suena
sintácticamente incorrecta.) Son condicionales subjuntivos
de la forma \say{Si $  p $ hubiera sido el caso, entonces $
q $ sería el caso.} Por ejemplo, estos condicionales nos
obligan a evaluar qué condiciones de verdad tiene la oración
\say{si México no hubiera sido conquistado por los
españoles, entonces México tendría cantidades
exorbitantes de oro.} Si la teoría define a la causalidad
en estos términos, entonces no tener criterios claros para
saber si estos condicionales son verdaderos o falsos
colapsa el proyecto de la teoría manipulabilista.

Parece más que natural implementar estos condicionales en
esta teoría de la causalidad. Una vez que hemos definido las
variables causalmente relevantes, y podemos
incluir tanto variables discretas como variables
continuas, entonces podríamos analizar condicionales del tipo
...
fuerza. Por ejemplo, en la página 81, Woodward nos dice que
\say{Debería ser obvio que una versión plausible de la
  teoría intervencionista de la causalidad necesita ser
formulada en términos contrafácticos.}\footnote{
    En el original \say{That a plausible version of an
    interventioonist theory of causation needs to be
    formulated in terms of counterfactuals should be
    obvious.}
}

Estos condicionales no sólo juegan un papel en la teoría
manipulabilista, sino que se han vuelto parte de los
\emph{modelos causales} que suelen hacerse en investigación
científica. Por ejemplo, en modelos
estadísticos---especialmente al hacer inferencia
causal---los condicionales \emph{contrafácticos} aparecen
como parte de este análisis. La \emph{inferencia causal} es
una herramienta usada para derivar conclusiones causales a
partir de agregación de datos e inferencia estadística.
Estos métodos sirven, sobre todo, para \emph{determinar}
cuáles factores son causalmente relevantes, de manera tal
que, al intervenir en la variable $A$, la variable $B$
%cambie en consecuencia \parencite{Pearl2016, Pearl2018,
llaudet2023}.

Una aplicación de este tipo de metodologías son las \emph{Pruebas Controladas
Aleatorizadas} (RCT, por sus siglas en inglés). En estas
pruebas---típicamente farmacéuticas---los investigadores
interesados en saber cuál es la efectividad de un
medicamento, intervienen en una de las variables---dándole
un placebo a un grupo de control---mientras que otro grupo
recibe el medicamento a testar. Asumiendo que no hay sesgos
en la muestra, si el grupo de prueba responde al
medicamento, entonces (en principio) seremos capaces de
detectar si el medicamento hace una aportación causal.
Podemos pensar a este tipo de pruebas, como si el grupo de
control funcionara como una especie de imagen alterna de lo
que \emph{habría} pasado de no haber tomado los
medicamentos.

Herramientas de este estilo, son usadas para aislar factores
causales a partir de agregados de datos e inferencia
estadística. Existe un interés particular por aislar
factores causales en investigación científica. Por
ejemplo, para saber qué factores afectan causalmente el
...
herramienta usada para \emph{modelar} este tipo de fenómenos
y, a partir de las relaciones entre las variables, descifrar
si en efecto existe una relación causal.

Supongamos que queremos saber si comer dulces \emph{causa}
que las personas tengan caries. En principio, podemos tomar
a una población de alumnos de una escuela primaria, y
dividir a la población en dos grupos: alumnos con caries y
alumnos sin caries. Es claro que cada uno de los individuos
en estos grupos tendrá diferentes características: la
higiene bucal, la deficiencia de calcio o la edad, que
también son variables que propician la aparición de caries.
Pero por ahora, queremos establecer una relación causal
entre comer dulces y tener caries. Entonces decidimos
dividir a la población bajo estudio en dos grupos: las
personas que consumen dulces y las personas que no.

De nuevo, es posible que en ambos grupos haya personas que
no siguen las instrucciones de sus dentistas y su higiene
bucal es precaria. Esto quiere decir que,---incluso si las
personas del grupo no comen dulces---si tienen pésima
higiene bucal, tendrán caries. Pero hay
casos en los que a pesar del consumo de azúcar, si tienen
una excelente higiene bucal, entonces algunas no tendrán
caries. Para asegurar que sólo estamos evaluando la variable
relevante (el consumo de dulces,) tanto las personas en el
grupo de control como el grupo de tratamiento son
seleccionados aleatoriamente de un conjunto de la
población.
...
personas tengan caries. En este caso, la variable que
estamos tratando de aislar es el consumo de dulces y si la
aparición de caries se debe al consumo de azúcar
\emph{independiente} de otras variables, entonces el consumo
de dulces \emph{causa} la aparición de caries. Al hacer
esto, estamos evaluando el contrafáctico \say{si los alumnos
hubieran consumido dulces, entonces tendrían caries.} Si
leemos tanto a las RCT y a la inferencia causal bajo esta
luz, creo que quedan claras las aplicaciones que estos
condicionales tienen en la investigación científica, algo
que además queda reflejado en
%\parencite{Pearl2016}, en \parencite{Pearl2018} y
%especialmente en \parencite{llaudet2023}.

Creo que, por ahora, esto es suficiente para motivar que los
condicionales contrafácticos encuentran su aplicación en
estos contextos para, digamos \say{crear} un evento alterno
para evaluar el condicional \say{si las personas no
hubieran consumido dulces, entonces no tendrían
caries.}\footnote{
...
%una conclusión causal \parencite{Otsuka2023}.
...
problemas filosóficos que acarrean. En particular, si la
teoría manipulabilista no sirve por sí sola para determinar
qué condicionales son verdaderos o falsos y si dichos
condicionales van a jugar un papel en la explicación
científica, entonces necesitamos de alguna manera resolver
este problema. Dicho de otra manera, hay que saldar la
\emph{deuda normativa}



\paragraph{En este trabajo} de investigación, me voy a
dedicar a tratar el problema de la normatividad. Tanto la teoría manipulabilista, como diferentes métodos en
investigación, definen la causalidad en términos de
\emph{condicionales contrafácticos.} Si esto es cierto, entonces hay que tener
claro por lo menos cómo \emph{justificar} las hipótesis en
las que se involucra algún tipo de relación causal
apelando a dichos condicionales. Mi proyecto, me parece,
tiene que satisfacer los desiderata que mencioné
anteriormente y, por tanto, ser cercano a cómo de hecho se
practica la investigación científica. Dado que esta tarea es
amplia, he decidido concentrar mis esfuerzos en el
problema de la causalidad, en particular \emph{explicaciones
causales} que hacen uso de \emph{condicionales
contrafácticos.} He decidido concentrarme en este tipo de
%explicaciones, porque  siguiendo a \parencite{Suarez2024} y
%\parencite{Ruyant2021}, es más fructífero analizar buena
parte de la ciencia si adoptamos una postura basada en
modelos. Las motivaciones detrás de esto las dejaré para el
capítulo siguiente, porque me interesa discutir el papel que
juega la representación en la explicación científica. Por el
momento quiero destacar que me parece que este trabajo
comparte muchas de las motivaciones de los autores
mencionados anteriormente. Mi enfoque para analizar a la
explicación científica sera a través de casos particulares
de explicación causal, en especial \emph{modelos causales.}
Me concentro en estos modelos porque de esta manera puedo
ofrecer un análisis más localizado, centrado en la práctica
científica, en especial la construcción de modelos
causales. Este énfasis en modelos, me permite además
discutir algunos de los compromisos que los investigadores
particulares tuvieron al construir sus modelos científicos,
lo que necesariamente implica incluir factores como las
limitaciones cognitivas, las pretensiones de los
investigadores al crear el modelo, además del uso
pretendido del modelo (para lo que fue diseñado.)

En filosofía de la ciencia más contemporánea, se suele
llamar \say{pragmáticas} a posturas que comparten estos
%desiderata \parencite{Suarez2024, Creath2023,
scientific-theories, Ruyant2021}. Si bien tanto Suárez como
Ruyant tienen como uno de sus objetivos articular una teoría
pragmática de la ciencia, me parece que aún no hay un
consenso obvio de qué cae bajo el término \emph{pragmático.}
Es por ello que he decidido señalar cuáles \emph{desiderata} quiero
satisfacer y qué otros supuestos estoy asumiendo. Sin embargo, pesar de no llamar \emph{pragmática} a la postura que
desarrollo en este trabajo, mi trabajo de investigación
comparte muchos de los compromisos de estas posturas. En
particular, creo que el enfoque en analizar modelos
científicos tiene una serie de ventajas que como filósofos
de la ciencia no podemos simplemente negar. Además, esto
permite analizar más de cerca el contexto histórico
involucrado en la construcción del modelo. Además, como
pretendo concentrarme en la relación que tiene el modelo con
el mundo (una representación adecuada), tomo parte de
%las distinciones hechas por \parencite{Ruyant2021} y
%\parencite{Suarez2024}

Me parece además que el problema que he descrito es
uno de representación, esto es, que es un problema que en su
núcleo lidia con cómo nuestras \emph{hipótesis causales
representan relaciones causales reales.} He decidido
concentrar mis esfuerzos en lidiar con la \emph{explicación
causal} porque buena parte de la investigación científica
lidia con relaciones causales, como he intentado mostrar en
las secciones anteriores de este capítulo. Esto sumado a que la
inferencia causal desarrolla modelos que tratan de capturar
la existencia de relaciones causales. Me parece además
que resolver el problema normativo, bajo los términos
que he descrito, equivale a señalar cómo es que
a partir de un modelo causal, estamos \emph{justificados} a
decir que hemos capturado una relación causal \say{real.}
Esta discusión la dejaré para el siguiente capítulo.

Mi objetivo principal es ofrecer un análisis de la
explicación científica que pueda responder al problema
normativo (cómo deberíamos razonar cuando hay contrafácticos
involucrados) Para llegar a esta conclusión haré una serie
de distinciones en el siguiente capítulo. Las distinciones
que ofrezco en ese capítulo no son novedosas, sino que
echo mano de distinciones hechas en filosofía de la
ciencia por el llamado \emph{empirismo lógico.} A primera
vista esto parece un regreso y no un progreso en torno a
entender la función de los modelos en explicación
científica, sin embargo, creo que la historia de la
filosofía se ha centrado en las tesis que defendió Carnap
(que tampoco son tan astringentes como se suelen presentar)
y muy poco en el desarrollo que tuvieron las posturas de
otros miembros del Círculo de Viena y cómo progresaron los
compromisos de los miembros, una vez que algunos de los
problemas con los que lidiaron, fueron adoptados en Estados
Unidos. Me parece que la novedad de este análisis radica en
presentar una imagen diferente del llamado \emph{empirismo
lógico.} A partir de esta distinciones extraigo la
conclusión del capítulo que es que para dar cuenta del éxito
de la explicación científica, debemos retomar la distinción
entre el \emph{contexto de descubrimiento} y el
\emph{contexto de justificación.}

Ofrecidas estas razones, comienzo el siguiente capítulo
señalando por qué la caracterización que ofrecí del
\emph{contexto de descubrimiento} y el \emph{contexto de
justificación} es muy cercana a los compromisos que tendría
una teoría pragmática en Filosofía de la Ciencia. Con estas
herramientas en mano, presento algunos modelos científicos
que, me parece, son más provechosamente analizados bajo las
distinciones que propongo.

El siguiente capítulo es el que posiblemente sea más
controversial para una teoría pragmática en filosofía de la
ciencia, pues lo dedico a motivar por qué la verdad juega un
papel central al analizar el éxito de las explicaciones
científicas. Si bien confesé que mis intuiciones son
causales, también son profundamente fregeanas. Es por ello
que defiendo que el éxito de las explicaciones científicas
depende de que los términos refieran a objetos reales. Me
parece además que hablar de representación en términos de
referencia y verdad, tal como los presento en este trabajo,
echa luz en torno al problema de la representación
\emph{modelo-mundo.} Por último, me dedico a evaluar el papel que juegan los
contrafácticos en la investigación científica. En especial
el papel que tienen en el razonamiento causal,---digamos la
extracción de información verdadera de una relación causal a partir de nuestros
supuestos---después presento el problema epistémico en
torno a la modalidad, en particular lo que  han llamado el
problema del acceso. La pregunta general de la epistemología
de la modalidad es es cómo justificamos razonamientos que involucran
modalidad. La pregunta particular acceso
epistémmico es ¿cómo podemos acceder epistémicamente al
%dominio de lo modal? \parencite{Vaidya2018} Lo que pretendo hacer es utilizar las
herramientas y distinciones que hago a lo largo del trabajo
y presento qué alternativas tenemos para resolver este
problema. Si la investigación científica explica a través de
condicionales contrafácticos, más vale tener una buena
caracterización de cuándo estamos justificados a afirmar que
un contrafáctico es verdadero.

Me queda claro que este
último capítulo no tiene un argumento original, sino que lo
presento para hacer explícitas algunas de las consecuencias
que tienen las tesis que defiendo a lo largo de mi trabajo.
La originalidad de este capítulo, me parece, descansa en
presentar bajo una luz diferente lo que ha recibido el nombre de \say{el
problema fundamental de la inferencia causal.}
%\parencite{Holland, llaudet2023, Otsuka2023} Dicho en una
oración: que no es posible saber el valor de verdad de un
condicional contrafáctico, porque lo que realmente podemos
observar son los resultados actuales, no los resultados
contrafácticos. Si por definición del condicional, estamos
evaluando algo que no sucede, pero que pudo haber sucedido:
¿exactamente qué evidencia podemos ofrecer para justificar
condicionales como \say{si los españoles no hubieran
conquistado México, entonces conservaríamos todas nuestras
reservas de oro}?

Este problema es un problema relacionado con la
epistemología de la modalidad. Para ejemplificar esto, supongamos que queremos saber si los estudiantes
...
sería peor.} \emph{Prima facie,} esto significa que tenemos que
...
digamos, qué hubiera sido de Ana si hubiera leído menos de un
...
Me parece que en el núcleo, este es un problema acerca del significado o valor de
verdad de condicionales contrafácticos. En particular
aquellos que cuantifican sobre individuos particulares y no
sobre un agregado de datos:
podemos hacer inferencias de cómo el grupo de prueba y el
...
entre la variable leer y un alumno particular.
...
%diferente a \emph{incentivar} a los estudiantes a leer. Por
%supuesto, quienes usan este tipo de métodos en su
%investigación tienen presente que con esto hemos perdido
%explicación de causas particulares \emph{token-token.}

Supongamos que hemos adquirido la
...
epistémico: sabemos que muchas buenas explicaciones
se han ofrecido a lo largo de la historia de las disciplinas
científicas, que han resultado inútiles a la luz de nueva
evidencia.

Para resolver estas tensiones, lo que yo voy a hacer es
trazar la normatividad de las explicaciones
causales---definidas a través de condicionales contrafácticos---en términos de
\emph{verdad.} Me parece que la interpretación que ofrezco,
resuelve muchas de las tensiones que tiene la discusión de
la explicación científica, mientras que al mismo tiempo se
mantiene cercana a la práctica de la investigación
científica.  Por ahora, basta señalar que el argumento
general es que si la verdad no juega un
papel en la investigación, entonces caeremos presa del
problema de reificar los
modelos como si fueran el fenómeno real
%\parencite{Winther2014, Lewontin2017, lewontin2006, Lewontin2021}.


%By analogy, suppose we discovered a toolbox and wanted to
%justify why it contains that specific set of tools. No one
%tool can do everything, and so we need several. Yet a worker
%shouldn’t be overladen, so one should also avoid unnecessary
%redundancy. There are trade-offs to be made in this respect,
%but reasons can be given for the choices made. Understanding
%a toolkit requires reference to the sorts of objects and
%materials the tools are designed to work on but also to the
%user(s) of the tools and their aims, needs, and abilities.

%Introduction From biological practice to metaphysics.
%However, as we have stated, a major aim of this volume is to
%explore the possibility of a metaphysics that is informed by
%practice. In fact, when we recognize scientific practice as
%the engine of success, the very possibility of a scientific
%metaphysics (based on our most successful science) implies
%the relevance of practical matters to metaphysical problems.


%\say{when a child lagged behid it's age-mates by as much as
%two years of mental age, it seemed obvious to Binet that
%remedial intervention was called for.} \parencite{Lewontin2017}
% TODO:
...
En las clases de maestría hubo profesores excelentes. Mucho
de lo que aprendí en estos cursos me sirvió para enmarcar de
diferente manera las preguntas que me preocupaban, lo cuál
me permitió entender --mejor, me parece-- la naturaleza de
la investigación científica. En particular quería
responder preguntas sobre la epistemología de la ciencia,
preguntas del tipo: \say{¿qué estamos justificados a
creer?,} \say{¿cuándo podemos afirmar que una hipótesis ha
sido corroborada?,} \say{¿es la explicación o compresión
más básica que el conocimiento?,} etc. Preguntas que
surgen a partir de mi interés por la explicación causal.

Ahondar en estas preguntas y tratar de responderlas es una
tarea más que complicada. Muchos de los obstáculos se
deben a que la práctica científica es más caótica y
heterogénea de lo que parece en un principio. Incluso si
nos concentramos en el uso de herramientas estadísticas o
herramientas que echan mano de la teoría de la
probabilidad, en casi todos los casos, ciertas propiedades
del \emph{fenómeno en cuestión} son idealizadas o
ignoradas para hacer que el modelo estadístico sea de
utilidad.

Esta representación parcial del fenómeno se debe a que en
muchos casos buscamos destacar algunas propiedades
particulares, sin que todas las variables estén
representadas en el modelo. Que no involucremos todas y cada
una de las variables que afectan el fenómeno es un reflejo
de nuestra limitada capacidad cognitiva. A lo largo de su
escrito, Potochnik nos recuerda que los modelos diseñados en
la investigación científica tienen un propósito particular.
Son usados para destacar algunas propiedades particulares,
especialmente, especialmente si lo que nos interesa son
...
%\textcite[p.24]{abrams2023evolution}, por ejemplo, nos
recuerda que \say{So-called infinite-population models are
simply models that have no role for drift. Biologists do
often say things about evolution in infinite populations,
and these claims are usually correct: the role of this
terminology \emph{in practice} implies that it should not
be understood literally. } (énfasis agregado)

A pesar de que los modelos no son completamente adecuados,
--en el sentido en el que no representan totalmente el
fenómeno en cuestión,-- son ampliamente utilizados en la
investigación científica. Si estas herramientas son usadas
ampliamente, entonces como filósofos de la ciencia hay que
ofrecer un análisis de cómo exactamente los modelos sirven
para justificar hipótesis si parten de supuestos obviamente
falsos.

Me parece que buena parte del problema está relacionado con
determinar cuál es el papel que juega la
\emph{representación adecuada} y la \emph{verdad} en la
investigación científica. Para motivar mi afirmación
anterior, quiero dar un repaso por un periodo de la
filosofía de la ciencia, un periodo en el cuál --suele
decirse-- la \emph{verdad} jugaba un papel central en los
análisis filosóficos de la ciencia: me refiero al periodo en
el que los miembros del Círculo de Vienna estuvieron vivos.
Este repaso sirve para motivar la afirmación de que la
\emph{verdad} juega un papel en la investigación científica
--por el momento no asumiré cuál es ese papel,-- mientras
que esto es al mismo tiempo compatible con la
\say{representación parcial} que hacen los modelos de un
fenómeno.

Para comenzar este repaso, debo señalar que en la maestría
aprendí nuevas metodologías de investigación, además de
diferentes maneras de plantear y entender las preguntas que
me preocupaban. Pero lo más valioso que aprendí fue la
importancia que tiene la historia de la ciencia en la
filosofía de la ciencia. Me voy a permitir hacer una breve
caracterización, exageradamente general, de dos posturas en
historia de la ciencia. El siguiente capitulo lo dedico a
caracterizar estas dos posturas y a señalar cómo las
distinciones entre el \emph{contexto de descubrimiento} y el
\emph{contexto de justificación} es crucial para responder a
estas inquietudes.
